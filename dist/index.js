...
module.exports = function (host, username, password, name, group, version) {
  ...
  const command = `IMAGEID=$(uuidgen) IMAGETAG="${host}/${group}/${name}:${version} docker build -t $IMAGETAG:$IMAGEID . && docker tag $IMAGETAG:$IMAGEID $IMAGETAG:${version} && echo '{"auths": ' >> config.json && curl -u ${username}:${password} -s "${host}/v2/auth" >> config.json && echo " }" >> config.json && sed -i 's%localhost:8081%${host}%' config.json && docker --config=./ push $IMAGETAG:${version}`;
...
const { compressDirectory, compressFiles } = __nccwpck_require__(458);
const { deployArtifactUrl, provisioningArtifactUrl } = __nccwpck_require__(88);
...
(() => {
const core = __nccwpck_require__(186);
const maven = __nccwpck_require__(391);
const docker = __nccwpck_require__(714);
